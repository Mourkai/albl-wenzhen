  ...
  methods: {
    async sendRequest() {
      ...
      try {
        ...
        const strBefore = `你是一名专业的坐诊医师，请基于大模型和应用里的知识库数据分析以下患者信息。请严格按照JSON格式输出，必须包含以下字段：
{
  "auxiliaryExamination": "辅助检查建议",
  "treatmentPrinciple": "治则",
  "diagnosisBasis": "辨病辨证依据",
  "treatmentPlan": "治疗方案",
  "tcmDiagnosis": [
    {
      "disease": "中医疾病名称",
...
请分析以下患者信息：`;
        ...
        const strAfter = `
请记住：
1. 必须以标准JSON格式输出，不用markdown
2. 每个字段一句话概括
3. tcmDiagnosis数组根据症状匹配程度返回至少1对(也可以返回多对)，按匹配度从高到低排序
4. tcmDiagnosis数组内容必须基于知识库数据`;
        ...
        try {
          // 尝试解析返回的文本为JSON
          const jsonResponse = JSON.parse(data.output.text)
          // 格式化JSON显示
          this.streamResponse = JSON.stringify(jsonResponse, null, 2)
          
          // 可以在这里添加代码来使用解析后的JSON数据
          // this.handleStructuredResponse(jsonResponse)
